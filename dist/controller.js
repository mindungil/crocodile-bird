...
아래 입력을 순화해주세요:
`;chrome.runtime.onMessage.addListener((t,n,e)=>{if(t.type=="crocodile-bird-clean")return(async()=>{try{console.log("서버와의 통신 시작");const o=await N(t.text,t.num);e({cleaned:o})}catch(o){console.error(`clean Text 실패: ${o}`),e({cleaned:t.text})}})(),!0});async function N(t,n){try{let e;n==1?e=E:(n==2||console.log("프롬프트 번호 수신 오류"),e=_),console.log(`message 프롬프트: ${e}`);const o=await fetch("http://localhost:3000/api/cleanText",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-4o-mini",messages:[{role:"system",content:e},{role:"user",content:t}],num:n})});if(!o.ok)throw new Error(`서버 응답 실패: ${o.status}`);const O=await o.json();return console.log("서버 응답 데이터:",O),O.cleaned}catch(e){return console.error(e),t}}
