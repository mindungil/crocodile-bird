...
아래 입력을 순화해주세요:
`;chrome.runtime.onMessage.addListener((o,n,e)=>{if(console.log("서버와의 통신 시작22"),o.type=="crocodile-bird-clean")return(async()=>{try{console.log("서버와의 통신 시작11");const t=await a(o.text,o.num);e({cleaned:t})}catch(t){console.error(`clean Text 실패: ${t}`),e({cleaned:o.text})}})(),!0});async function a(o,n){try{let e;n==1?e=r:n==2?e=c:n==3?e=s:(console.log("프롬프트 번호 수신 오류"),e=prompts.level_3),console.log(`message 프롬프트: ${e}`);const t=await fetch("http://localhost:3000/api/cleanText",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-4o",messages:[{role:"system",content:e},{role:"user",content:o}],num:n})});if(!t.ok)throw new Error(`서버 응답 실패: ${t.status}`);const l=await t.json();return console.log("서버 응답 데이터:",l),l.cleaned}catch(e){return console.error(e),o}}
